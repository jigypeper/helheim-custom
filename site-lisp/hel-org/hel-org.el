;;; hel-org.el --- Hel integration with Org-mode -*- lexical-binding: t; -*-
;;
...
;; Package-Requires: ((emacs "29.1") (hel "0.12.0") (org "9.7") (dash "2.19.1"))
...
;;
;; Org 9.7 or newer is required: the element API this file builds on
;; (`org-element-begin' and friends) was introduced there.  Emacs 30.1
;; bundles it; on Emacs 29 install Org from GNU ELPA.
;;
;;; Code:
...
(add-hook 'org-mode-hook 'hel-org-surround-settings)
...
cursors in the buffer."
  (when org-hide-emphasis-markers
    (hel-keymap-set org-mode-map
      "<remap> <hel-select-regex>"      'hel-org-select-regex       ; s
      "<remap> <hel-split-region>"      'hel-org-split-region       ; S
      "<remap> <hel-copy-selection>"    'hel-org-copy-selection     ; C
      "<remap> <hel-copy-selection-up>" 'hel-org-copy-selection-up) ; M-c
...
;; s
(hel-define-command hel-org-select-regex (&optional invert)
...
entered regexp withing current selections."
  ...
  (let ((org-hide-emphasis-markers nil))
    (font-lock-flush)
    (hel-select-in-selections invert)))

;; S
(hel-define-command hel-org-split-region ()
  "Split each selection according to the regexp entered."
  :multiple-cursors nil
  (interactive)
  (hel-org-select-regex t))
...
(defun hel-org-surround-settings ()
